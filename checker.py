...
def get_meest_status(track):
    try:
        ...
        # План Б: HTML парсинг если API не пустил
...
def get_np_status(track):
    try:
        ...
        # Отправляем через files= чтобы имитировать multipart/form-data
        payload = {
            'token': (None, token),
...
try:
    ...
    for i, row in df.iterrows():
        ...
        if "Мист" in str(carrier):
            ...
            new_status = get_np_status(track) # ТУТ ТЕПЕРЬ ПРАВИЛЬНОЕ ИМЯ
        else:
    ...
    repo.update_file(file.path, f"Sync update {now}", df.to_csv(index=False), file.sha)
